...
const app = {
    ...
    renderAdminUsers() {
        ...
        pending.forEach(user => {
            ...
            <tr class="bg-amber-50 dark:bg-amber-900/10 border-b border-border-light dark:border-border-dark last:border-0">
                <td class="px-4 py-3 font-bold text-amber-600 dark:text-amber-400">
                    ${user} <span class="ml-2 text-[10px] uppercase bg-amber-200 dark:bg-amber-900 px-2 py-0.5 rounded text-amber-800 dark:text-amber-200">Request</span>
                </td>
                <td class="px-4 py-3 text-text-muted-light dark:text-text-muted-dark text-xs">Waiting Approval</td>
                <td class="px-4 py-3 text-right">
                    <button onclick="app.adminApproveUser('${user}')" class="text-xs font-bold text-win hover:underline mr-3">Approve</button>
                    <button onclick="app.adminRejectUser('${user}')" class="text-xs font-bold text-loss hover:underline">Reject</button>
                </td>
            </tr>`;
        });
        ...
        // Active Users
        users.forEach(u => {
            const permissionCount = u.permissions.length;
    ...
    saveSettings() {
        ...
        if (key) SafeStorage.set(key, JSON.stringify(this.settings));
    },
    ...
    renderHabits() {
        ...
        const checklist = document.getElementById('habit-checklist-container');

        if (!listCues) return; // Guard clause if view not loaded

        // Clear Lists
        listCues.innerHTML = '';
        listCravings.innerHTML = '';
        listEase.innerHTML = '';
        listRewards.innerHTML = '';
        checklist.innerHTML = '';
        ...
        this.habits.forEach(habit => {
            // Populate Atomic Lists
            if (habit.cue) listCues.innerHTML += `<li>${habit.cue}</li>`;
            if (habit.craving) listCravings.innerHTML += `<li>${habit.craving}</li>`;
            if (habit.name) listEase.innerHTML += `<li>Reduce friction for: <strong>${habit.name}</strong></li>`;
            if (habit.reward) listRewards.innerHTML += `<li>${habit.reward}</li>`;

            // Populate Checklist
            const isCompleted = log.includes(habit.id);
            checklist.innerHTML += `
                <div class="flex items-center justify-between p-3 rounded-xl border ${isCompleted ? 'border-emerald-500/50 bg-emerald-500/10' : 'border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark'} transition-all cursor-pointer" onclick="app.toggleHabit(${habit.id})">
                    <div class="flex items-center gap-3">
                        <div class="w-6 h-6 rounded-full border-2 ${isCompleted ? 'bg-emerald-500 border-emerald-500' : 'border-slate-400'} flex items-center justify-center transition-all">
                             ${isCompleted ? '<span class="material-symbols-outlined text-white text-sm">check</span>' : ''}
                        </div>
                        <span class="${isCompleted ? 'line-through text-slate-500' : 'font-medium'}">${habit.name}</span>
                    </div>
        ...
        if (this.habits.length === 0) {
            checklist.innerHTML = '<div class="text-center text-slate-500 text-sm py-4">No habits yet. Add one!</div>';
        }
        ...
        if (streakEl) streakEl.textContent = this.habitStreak;
    },
    ...
    calculateStreak() {
        // Calculate Streak: Consecutive days where AT LEAST ONE habit was checked? 
        // Or all habits? Let's go with "at least one" for now, or improved logic: 
        // Count days in a row backwards from yesterday/today that exist in logs.
        ...
        // Check if today is done, if not, check yesterday
        const today = new Date().toISOString().split('T')[0];
